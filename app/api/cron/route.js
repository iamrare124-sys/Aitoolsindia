...
export async function GET(request) {
  ...
  try {
    ...
    // Build excerpt — sanitize + 3-level fallback chain
    function sanitizeExcerpt(str) {
      if (!str) return ''
      return str
        .replace(/&lt;[^&]*&gt;/g, '')   // &lt;a href...&gt;
        .replace(/&lt;/g, '').replace(/&gt;/g, '').replace(/&amp;/g, '&')
        .replace(/https?:\/\/\S+/g, '') // bare URLs
        .replace(/href=["'][^"']*["']/gi, '') // href= fragments
        .replace(/\s+/g, ' ').trim()
    }
    const rawExcerpt =
      generated.metaDesc ||
      generated.content?.hook ||
      generated.content?.sections?.[0]?.body?.substring(0, 200) ||
      ''
